...
public class GirafeCameraFollow : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        if (target != null && target.gameObject.activeSelf)
        {
            ...
            Vector3 targetPositionVertical = target.position + offset;
            targetPosition.y = transform.position.y;
            targetPositionVertical.x = transform.position.x;

            Vector3 newPosition = Vector3.Lerp(targetPositionVertical, targetPosition, smoothSpeed);
